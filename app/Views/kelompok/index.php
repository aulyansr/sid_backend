        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-striped compact" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php foreach ($kelompok as $index => $kelompok) : ?>
                            <tr>
                                <td align="center"><?= $index + 1; ?></td>
                                <td>
                                    <div class="btn-group">
                                        <a href="<?= site_url('admin/kelompok/' . esc($kelompok['id'])); ?>" class="btn btn-sm btn-secondary" title="Lihat Data">
                                            <i class="fa fa-eye"></i> Lihat
                                        </a>
                                        <a href="<?= site_url('admin/kelompok/' . esc($kelompok['id']) . '/edit'); ?>" class="btn btn-sm btn-warning" title="Ubah Data">
                                            <i class="fa fa-edit"></i> Ubah
                                        </a>

                                        <form action="<?= base_url('admin/kelompok/' . esc($kelompok['id'])); ?>" method="post" style="display:inline;">
                                ...
                                <td><?= esc($kelompok['nama']); ?></td>
                                <td><?= esc($kelompok['ketua']); ?></td>
                                <td><?= esc($kelompok['jenis_kelompok']); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<script>
    $(document).ready(function() {
        const table = $("#dataTable").DataTable({
            lengthChange: false,
            buttons: [{
                    text: `<i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;Atur Kelompok Master Baru`,
                    className: "btn-sm",
                    action: function(e, dt, node, config) {
                        window.location.href = '/admin/master-kelompok/';
                    },
                },
                {
                    text: `<i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;Tambah Kelompok Baru`,
                    className: "btn-sm",
                    action: function(e, dt, node, config) {
                        window.location.href = '/admin/kelompok/new';
                    },
                },
                {
