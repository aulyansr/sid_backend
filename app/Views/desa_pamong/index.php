...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Manajemen Pengurus</h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-striped compact" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php foreach ($desaPamongs as $index => $pamong) : ?>
                            <tr>
                                <td align="center"><?= $index + 1; ?></td>
                                <td>
                                    <div class="uibutton-group">
                                        <a href="/admin/pengurus/<?= esc($pamong['pamong_id']); ?>/edit" class="btn btn-sm btn-warning" title="Ubah Data">
                                            <i class="fa fa-edit"></i> Ubah
                                        </a>
                                        <form action="<?= base_url('admin/pengurus/' . esc($pamong['pamong_id'])); ?>" method="post" style="display:inline;">
                                ...
                                <td><?= esc($pamong['pamong_nama']); ?></td>
                                <td><?= esc($pamong['pamong_nip']); ?></td>
                                <td><?= esc($pamong['pamong_nik']); ?></td>
                                <td><?= esc($pamong['jabatan']); ?></td>
                                <td><?= esc($pamong['pamong_status']); ?></td>
                                <td><?= esc($pamong['pamong_tgl_terdaftar']); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<script>
    var newPamongPath = '/admin/pengurus/new';

    $(document).ready(function() {
        const table = $("#dataTable").DataTable({
            lengthChange: false,
            buttons: [{
                    text: `<i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;Tambah Pengurus Baru`,
                    className: "btn-sm",
                    action: function(e, dt, node, config) {
                        window.location.href = newPamongPath;
                    },
                },
                {
